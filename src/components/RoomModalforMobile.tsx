...
const RoomModalforMobile: React.FC<RoomModalforMobileProps> = ({ initialRooms }) => {
    ...
    return (
        <div className='row-start-1 row-end-3 md:hidden lg:hidden xl:hidden'>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-10 flex items-center justify-center bg-black border bg-opacity-25">
                    <div className="bg-white rounded-2xl w-[50%] p-6 shadow-xl">
                        <h3 className="text-lg font-medium leading-6 text-gray-900">Rooms</h3>
                        <div className="mt-2">
                            {rooms.map(renderRoomLink)}
                        </div>
                        <button onClick={() => setIsOpen(false)} className="mt-4 text-white bg-red-500 rounded-md">Close</button>
                    </div>
                </div>
            )}
        </div>
